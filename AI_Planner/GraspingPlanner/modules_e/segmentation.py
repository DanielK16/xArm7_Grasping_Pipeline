...
class SamAnalyzer:
    CONTAINER_LABELS = ["bowl", "cup", "plate", "tray", "box", "pan", "bowl_orange", "bowl_green"]
    ...
    def _get_depth_stats(self, mask, depth_img):
            ...
            # Perzentil filter: We remove the first and last 5 % of values to remove outliner
            ...
            for cont in objects:
                ...
                if len(contains_list) > 1:
                    ...
                    if len(valid_items) >= 2:
                        ...
                        # ignore differences between 4mm
                        if delta_mm > 4:
    ...
    def _calc_overlap(self, inner, outer):
        """
        Computes percentage of overlap of two masks (1.0 = 100%) using bitwise AND operation
        """
    ...
    def analyze_scene(self, image, depth_img, objects): 
        ...
        results = self.model(image, bboxes=bboxes, verbose=False)
            # Safety Check
        if not results or results[0].masks is None: return objects, "SAM failed, No bounding boxes created"
            # convert masks to numpy and load it into cpu
        masks_data = results[0].masks.data.cpu().numpy()
            # save width and height of mask
        h, w = image.shape[:2]
        ...
        # 2.1 FIRST: Create the FILLED mask (the logical base)
            # We use convex hull to ensure we have a solid base for containers
            if is_container:
                mask_filled_u8 = self._fill_holes_convex(mask_u8)
            else:
                mask_filled_u8 = mask_u8

        # 2.2 SECOND: Customize the mask for grasping BASED ON the filled mask
            # Check for Cups -> Make Bigger
            if "cup" in label_lower:
                mask_grasp_u8 = self._make_cup_bigger(mask_filled_u8)
                
            # Check for Bowls/Plates -> Make Ring Mask from the filled version
            elif any(x in label_lower for x in ["bowl","bowl_orange","bowl_green", "plate"]):
        ...
        # 2.3 Compute median depth of the customized grasp mask
            median_z = self._get_depth_stats(mask_grasp_u8, depth_img)
            objects[i]['depth_median'] = median_z

            # We store boolean masks in 'processed' (not in 'objects' to avoid JSON error)
        ...
        for i, inner in enumerate(objects):
            # We check the logic mask of the item against the filled interior of others
            ...
            for j, outer in enumerate(objects):
                ...
                # compute which object is larger!
